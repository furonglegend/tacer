...
def parse():
    ...
    parser.add_argument("--mode", type=str, default="auto", required=True, 
                        choices=['auto', 'manual', 'sali', 'brev', 'fait', 'rand',
                                 'abl_sali', 'abl_brev', 'abl_fait',
                                 'joint_vae', 'adv', 'dsl', 'causal', 'embodied', 'evolve'],
                        help="the mode of the pipeline")
    
    # New module-specific arguments
    parser.add_argument("--vae_beta", type=float, default=1.0, help="Beta weight for VAE KL divergence")
    parser.add_argument("--adv_lambda", type=float, default=0.01, help="Lambda for adversarial length penalty")
    parser.add_argument("--evolve_pop_size", type=int, default=256, help="Population size for evolution")
    parser.add_argument("--evolve_generations", type=int, default=30, help="Number of generations for evolution")
...
if __name__ == "__main__":
    ...
    elif args.mode == "joint_vae":
        from trainers.joint_vae_trainer import run_joint_vae
        run_joint_vae(args)

    elif args.mode == "adv":
        from trainers.adv_trainer import run_adv
        run_adv(args)

    elif args.mode == "dsl":
        from trainers.dsl_trainer import run_dsl
        run_dsl(args)

    elif args.mode == "causal":
        from trainers.causal_trainer import run_causal
        run_causal(args)

    elif args.mode == "embodied":
        from trainers.embodied_trainer import run_embodied
        run_embodied(args)

    elif args.mode == "evolve":
        from trainers.evolve_trainer import run_evolve
        run_evolve(args)
